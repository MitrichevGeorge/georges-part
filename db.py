...
class AppDB(AutoSaveModel):
    ...
    def update_user(
        ...
        user = self.get_user_by_login(old_login)
...
class AsyncSqliteDocumentORM:
        ...
        with sqlite3.connect(self.filepath) as conn:
            ...
            if row:
                ...
                try:
                    ...
                    return AppDB.model_validate_json(
                        row[0]
                    )
    ...
    def _write_to_sql_sync(
        ...
        with sqlite3.connect(self.filepath) as conn:
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO document_store
                (key, value)
                VALUES ('root', ?)
                """,
                (payload,)
...
initedDB = AsyncSqliteDocumentORM(
    DB_FILE
)
...
async def main():
    ...
    initedDB.add_new_user(user)


    user = initedDB.get_user_by_login(
        "test"
    )
